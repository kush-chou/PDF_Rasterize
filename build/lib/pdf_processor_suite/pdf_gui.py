...
class MainWindow(QMainWindow):
    ...
    def create_split_tab_ui(self):
        ...
        # --- Input/Output Paths ---
        self.input_path_edit = QLineEdit(self.config.get("input_path", ""))
        self.output_path_edit = QLineEdit(self.config.get("output_path", ""))
        ...
        form_layout.addRow("Input PDF:", input_layout)
        ...
        form_layout.addRow("Resolution:", self.dpi_spinbox)

        self.split_level_spinbox = QSpinBox()
        self.split_level_spinbox.setRange(0, 10)
        self.split_level_spinbox.setValue(self.config.get("max_split_level", 0))
        form_layout.addRow("Max Split Level (0=unlimited):", self.split_level_spinbox)
        ...
        form_layout.addRow(self.rasterize_checkbox)

        self.keep_originals_checkbox = QCheckBox(
            "Keep original split PDFs (if rasterizing)"
        )
        self.keep_originals_checkbox.setChecked(
            self.config.get("keep_originals", False)
        )
        form_layout.addRow(self.keep_originals_checkbox)

        self.flatten_output_checkbox = QCheckBox("Flatten output directory structure")
        self.flatten_output_checkbox.setChecked(
            self.config.get("flatten_output", False)
        )
        form_layout.addRow(self.flatten_output_checkbox)
        ...
        self.split_start_btn = QPushButton("Start Splitting")
    ...
    def create_merge_tab_ui(self):
        ...
        self.merge_dir_edit = QLineEdit(self.config.get("merge_dir", ""))
        ...
        merge_dir_layout.addWidget(browse_merge_btn)
        form_layout.addRow("Folder to Merge:", merge_dir_layout)

        self.recreate_bookmarks_chk = QCheckBox(
            "Re-create bookmarks from _bookmarks.json"
        )
        self.recreate_bookmarks_chk.setChecked(
            self.config.get("recreate_bookmarks", True)
        )
        form_layout.addRow(self.recreate_bookmarks_chk)
    ...
    def start_split_process(self):
        ...
        self.config["max_split_level"] = self.split_level_spinbox.value()
        ...
        worker = Worker(
            ...
            self.keep_originals_checkbox.isChecked(),
            False,  # dry_run is not a GUI option for now
            self.config.get("workers"),
            self.config,
            self.flatten_output_checkbox.isChecked(),
            self.split_level_spinbox.value(),
    ...
    def start_merge_process(self):
        ...
        if not merge_dir:
            self.show_error("Input Error", "Please specify the folder to merge.")
            return
        ...
        self.config["merge_dir"] = merge_dir
        self.config["recreate_bookmarks"] = self.recreate_bookmarks_chk.isChecked()
