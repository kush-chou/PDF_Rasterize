...
When you run the "Split & Rasterize" process, the output folder will have the following structure:
...
│   └── ...
└── rasterized/
    ├── Chapter_1_rasterized.pdf
    ├── Chapter_2_rasterized.pdf
    └── ...
```

*   **unrasterized/**: Contains the initial PDFs that are split directly from the source document based on its bookmarks. If you do not select the "Keep Originals" option, this folder will be automatically deleted after the rasterization process is complete.
*   **rasterized/**: Contains the final, rasterized versions of the split PDFs. These are the files that are ready to be merged.

This structure keeps the intermediate and final files neatly organized.
...
*   `--flatten-output`: When splitting without rasterizing, this option saves all split files into a single directory. When rasterizing, the output is always flat within the `unrasterized` and `rasterized` subdirectories.
